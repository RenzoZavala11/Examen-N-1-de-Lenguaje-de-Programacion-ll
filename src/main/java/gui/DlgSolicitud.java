...
public class DlgSolicitud extends JDialog implements ActionListener {
	...
	void cargarActividades() {
		...
		try {
			...
			for (Actividad actividad : lstActividad) {
	...
	void listar() {
		...
		try {
			...
			List<Solicitud> lstSolicitud =
					manager.createQuery(jpql, Solicitud.class).getResultList();
			...
			for (Solicitud solicitud : lstSolicitud) {
				...
				imprimir("Fecha Registro......: " + solicitud.getFechaReg());
				imprimir("Actividad...........: " + solicitud.getActividad().getDescripcion());
				imprimir("Fecha Inicio........: " + solicitud.getActividad().getFechaInicio());
				imprimir("Nro Vacantes........: " + solicitud.getActividad().getNroVacantes());
				imprimir("Categoria...........: " + solicitud.getActividad().getCategoria().getDescripcion());
	...
	void adicionar() {
		...
		solicitud.setArchivoAdjunto(archivo);
	...
	void buscar() {
		Integer idSolicitud =Integer.parseInt(txtIdSolicitud.getText());

		EntityManager manager = JPAUtil.getEntityManager();

		try {
			Solicitud solicitud =
					manager.find(Solicitud.class, idSolicitud);
			if (solicitud == null) {
				mensajeError("Solicitud no encontrada");
				return;
			}
			txtArchivoAdjunto.setText(solicitud.getArchivoAdjunto());
			txtFechaRegistro.setText(solicitud.getFechaReg().toString());
	...
	void modificar() {
		...
		try {
			...
			solicitud.setActividad((Actividad) cboActividad.getSelectedItem());
